...
class Trainer:
    ...
    def _step(
        ...
        # Set model to training mode explicitly when in training phase
        if phase == 'train':
            model.train()
            outputs = model(batch)
        # Debug outputs to make sure they require gradients
        if phase == 'train':
            if isinstance(outputs, dict):
                for k, v in outputs.items():
                    if isinstance(v, torch.Tensor) and not v.requires_grad:
                        logging.warning(f"Output tensor {k} doesn't require gradients")
            elif isinstance(outputs, torch.Tensor) and not outputs.requires_grad:
                logging.warning("Model output doesn't require gradients")
    ...
    def train_epoch(self, dataloader, phase=Phase.TRAIN):
        batch_time = AverageMeter("Batch Time", self.device, ":.2f")
        data_time = AverageMeter("Data Time", self.device, ":.2f")
        mem = MemMeter("Mem (GB)", self.device, ":.2f")
        ...
        for model in curr_models:
            model.train()
            if hasattr(unwrap_ddp_if_wrapped(model), "on_training_epoch_start"):
                unwrap_ddp_if_wrapped(model).on_training_epoch_start()
        ...
        for data_iter, batch in enumerate(dataloader):
            # Update schedulers
            where_value = min((self.epoch + (data_iter / iters_per_epoch)) / self.max_epochs, 0.9999)

            step_number = int(self.epoch * iters_per_epoch + data_iter)

            self.optim.step_schedulers(where_value, step_number)
            # Log learning rates - modified to handle different optimizer structures
            if data_iter % self.logging_conf.log_scalar_frequency == 0:
                for i, param_group in enumerate(self.optim.optimizer.param_groups):
                    self.logger.log(
                        os.path.join("lr", f"group_{i}"), 
                        param_group['lr'],
                        self.steps[phase]
                    )
            ...
            # Run the forward / backward pass
            # Modify to capture the return value indicating if backward was skipped
            backward_skipped = self._run_step(batch, phase, loss_mts, extra_loss_mts)
            
            # Only perform optimizer steps if backward wasn't skipped
            if not backward_skipped:
                # Gradient clipping
                if self.gradient_clipper is not None:
                    self.scaler.unscale_(self.optim.optimizer)
                ...
                if self.gradient_logger is not None:
                    self.scaler.unscale_(self.optim.optimizer)
                    self.gradient_logger(self.model)
        ...
        for model in curr_models:
            if hasattr(unwrap_ddp_if_wrapped(model), "on_training_epoch_end"):
                unwrap_ddp_if_wrapped(model).on_training_epoch_end()
    ...
    #     with torch.amp.autocast(  # Updated to new style
    #         'cuda',
    ...
    #     # After computing loss but before backward
    #     if not loss.requires_grad:
    #         logging.error("Loss tensor doesn't require gradients. Debugging info:")
    #         logging.error(f"Loss requires_grad: {loss.requires_grad}")
    #         logging.error(f"Loss grad_fn: {loss.grad_fn}")

    #         # Check model parameters
    #         trainable_params = sum(p.requires_grad for p in self.model.parameters())
    #         total_params = sum(1 for _ in self.model.parameters())
    #         logging.error(f"Trainable parameters: {trainable_params}/{total_params}")

    #         # Create a dummy loss connected to model parameters
    #         logging.warning("Skipping backward pass for this batch since loss doesn't require gradients")
            
    #         # Update metrics even though we're skipping backward
    #         loss_mts[loss_key].update(loss.item(), batch_size)
    ...
    #         # Skip the rest of this function without calling backward
    #         return
    ...
    def _run_step(
        ...
        # Only do backward if loss requires gradients
        self.scaler.scale(loss).backward()
